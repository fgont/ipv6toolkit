/*
 * ns6 v1.1: A security assessment tool for attack vectors based on
...
unsigned char	 	verbose_f=0, sllopt_f=0, sllopta_f=0, iface_f=0, srcprefix_f=0, targetprefix_f=0;
...
int main(int argc, char **argv){
	...
	if(pcap_compile(pfd, &pcap_filter, PCAP_NOPACKETS_FILTER, PCAP_OPT, PCAP_NETMASK_UNKNOWN) == -1){
 ...
 */
void print_help(void){
	puts( "ns6 version 1.1\nAssessment tool for attack vectors based on NS messages\n");
	usage();
